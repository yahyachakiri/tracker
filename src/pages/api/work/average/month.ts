...
import { nextMonth, ErrorFromApi } from '@utils/index';
...
async function getDatabaseDataWithParam({
    res,
    date,
}: {
    res: IResponse;
                    ...
                    and: [
                        {
                            property: 'Date',
                            date: {
                                ...(date && { on_or_after: `${date.year}-${date.month}-01` }),
                                is_not_empty: true,
                            },
                        },
                        {
                            ...
                            date: {
                                ...(date && {
                                    before: `${date.nextYear}-${date.nextMonth}-01`,
                                }),
                                is_not_empty: true,
                            },
                        },
...
export default async function handler(req: NextApiRequest | any, res: NextApiResponse) {
    ...
    if (!date) res.status(404).json(new ErrorFromApi('Choose a valid date', 404));
    await getDatabaseDataWithParam({ res, date: nextMonth(date, res) });
}
